...
import { Clock, CheckCircle2, XCircle, KeyRound, ChevronDown, ChevronUp, CalendarClock } from 'lucide-react';
...
export default function KeywordNotificationMonitor() {
  const [expanded, setExpanded] = useState(false);

  const { data: queue = [] } = useQuery({
    queryKey: ['notification-queue-keywords'],
    queryFn: () => base44.entities.NotificationQueue.list('-created_date', 100),
    refetchInterval: 15000,
  });

  // Filtra notificações de palavra-chave: têm scheduled_at E contêm palavras-chave típicas
  const keywordQueue = queue.filter(n =>
    n.scheduled_at && (
      (n.message || '').toLowerCase().includes('palavra-chave') ||
      (n.message || '').toLowerCase().includes('encomenda') ||
      (n.message || '').toLowerCase().includes('portaria')
    )
  );

  if (keywordQueue.length === 0) return null;

  const pending = keywordQueue.filter(n => n.status === 'pending');
  const failed  = keywordQueue.filter(n => n.status === 'failed');
  const sent    = keywordQueue.filter(n => n.status === 'sent');

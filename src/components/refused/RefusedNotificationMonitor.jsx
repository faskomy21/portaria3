...
import { Clock, CheckCircle2, XCircle, PackageX, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function RefusedNotificationMonitor() {
  const [expanded, setExpanded] = useState(false);

  const { data: queue = [] } = useQuery({
    queryKey: ['notification-queue-refused'],
    queryFn: () => base44.entities.NotificationQueue.list('-created_date', 100),
    refetchInterval: 10000,
  });

  // Filtra apenas notificações SEM scheduled_at (recusadas e entregas são imediatas)
  // e que contenham palavras-chave típicas de recusa
  const refusedQueue = queue.filter(n => !n.scheduled_at && (
    (n.message || '').toLowerCase().includes('recus') ||
    (n.message || '').toLowerCase().includes('avari') ||
    (n.message || '').toLowerCase().includes('transportadora')
  ));

  if (refusedQueue.length === 0) return null;

  const pending = refusedQueue.filter(n => n.status === 'pending');
  const failed  = refusedQueue.filter(n => n.status === 'failed');
  const sent    = refusedQueue.filter(n => n.status === 'sent');
      ...
      {expanded && (
        <div className="space-y-2 mt-3 max-h-64 overflow-y-auto pr-1">
          {visible.map((n) => {
            ...
            const createdDate = toUTC(n.created_date);
